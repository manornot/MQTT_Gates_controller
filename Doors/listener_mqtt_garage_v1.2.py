...
status_topic = '/status'
topics = [str(room_number)+building+]
...
def main(gates):
    client_data = {'doors': gates}
    clnt = client.Client(client_id='319B', userdata=client_data)
...
def on_message(client, userdata, message):
    if DEBUG:
        print('DEBUG', 'message received = ', message.payload, time.time())
    sys.stdout.flush()
...
def nsc():
    if DEBUG:
        print('DEBUG', 'i have no idea what do you want from me')
    sys.stdout.flush()
    pass
...
def servetor_ai(gate, msg):
    #new_command = 0
    if DEBUG:
        print('DEBUG', 'servetor is online')
    if DEBUG:
        print('DEBUG', 'command received = ', msg)
    if DEBUG:
        print('DEBUG', servetor.get(msg, nsc))
...
def open_command(gate):
    ...
    # time.sleep(1)
    if DEBUG:
        print('DEBUG', 'Open this fucking gates! motherfucker!')
...
try:
    ...
    if DEBUG:
        print('DEBUG', 'exterminate')
    if DEBUG:
        print('DEBUG', 'the dead are useless')
    pass
