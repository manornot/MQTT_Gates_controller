...
def open_command(gate):
    if DEBUG: print('DEBUG','Roger Roger! Execute openning protocol!')
    #time.sleep(1)
    if DEBUG: print('DEBUG',gate.state)
    if gate.state == 'close' or gate.state == 'stop':
        if DEBUG: print('DEBUG','push me!')
        button_press()
def close_command(gate):    
    if gate.state == 'open':
           button_press()
    elif gate.state == 'stop':
        if DEBUG: print('DEBUG',gate.prev_state)
...
def button_press():
    #pass
    if DEBUG: print('DEBUG','open this fucking doors')
    gpio.output(11,1)
    time.sleep(1)
    gpio.output(11,0)
...
def on_message(client, userdata, message):
    ...
    if DEBUG: print('DEBUG',userdata.get('gates').state)
    sys.stdout.flush()
    if(any(cmd == message.payload for cmd in available_commands)):
        if DEBUG: print('DEBUG','its true')
...
def filter_pins(pin_states,win,win_len,dig):
    ...
    if DEBUG: print('DEBUG',motion_dir)
...
def servetor_ai(gate,msg):
    #new_command = 0
    if DEBUG: print('DEBUG','servetor is online')
    if DEBUG: print('DEBUG','command received')
